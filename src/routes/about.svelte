...
<div class="wrapper">
	...
	<p>
		...
		<em>ridiculously</em>
		fun, especially as they get bigger and start to play. The next best
		thing to having them in your house is at least being able to check in on
		them at any time. We had a puppy cam available for Disa's first litter
		and decided to do it again for this one.
	</p>

	<p>
		If history is any indicator, morning time—around 8am MST, say—will be a
		good time to see puppy activity. That’s when I (Jared) jump in with the
		puppies to get my fix before starting work.
	</p>
	...
	<p>
		...
		site, which means that everything “interesting” actually runs in your
		browser, and the hosting server (<code>puppycam.turehounds.com</code>)
	...
	<p>
		If you want to see the source for this site, you could, theoretically,
		as it’s hosted on GitHub. It’s currently marked as a private repository,
		though, so you can’t actually see it. Please let me know if you’re
		curious, and I could be persuaded to make it publicly visible. (There’s
		really nothing secret in the source; I just didn’t want to have random
		folks visiting the PuppyCam site.)
	</p>
	...
	<p>
		...
		cameras, for security. We use them so that we can easily keep an eye on
		things, including being able to peek in at the pups at night-time—the
		cameras use infrared in low-light situations so we don’t need to turn on
		a light just to check on them. You can also, as a camera owner, choose
		to make the feed from the camera “public”, which allows you to share
		a public link to the feed on the
		<code>nest.com</code>
		site,
		<em>or</em>
	...
	<p>
		For PuppyCam, I use a little bit of HTML/CSS magic to make one of the
		videos look like it’s “inset” into the other.
	</p>

	<h3 id="puppy-details">Puppy details</h3>

	<p>
		The details about the puppies is just content that I’ve typed into the
		site by hand… there’s nothing magic or particularly technical for this
		part. It would be awesome if the puppy weights could auto-update, but
		that would mean some kind of weight-measuring pad, being able to track
		which puppy is on the pad at any given moment, and so on… I mean, that
		would be
		<em>undeniably</em>
	...
	<p>
		...
		temperature and humidity sensors. They allow me to monitor such critical
		things as
		<em>“does my cigar humidor need more water?”</em>
		and
		<em>“what temperature is the kitchen right now?”</em>. These devices
		record the temperature and humidity once every minute, and send that
		information (via a low-energy-Bluetooth–to–WiFi hub) up to SenorPush’s
		data servers in the cloud.
	</p>
	...
	<p>
		...
		<em>outside</em>
		weather (as a nice contrast with the puppy weather), but that doesn’t
		seem really necessary—not that this entire site is really necessary. 😉
	</p>
</div>
